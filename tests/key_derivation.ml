...
let hash = Ciphersuite.hash13 cipher
...
let derive_c_hs_traffic () =
  ...
  | Some t ->
    let c_hs_traffic = Handshake_crypto13.derive_secret t "c hs traffic" log in
    Alcotest.check cs __LOC__ c_hs_traffic_secret c_hs_traffic ;
    let key, iv = Handshake_crypto13.traffic_key cipher c_hs_traffic in
...
let derive_s_hs_traffic () =
  ...
  | None -> Alcotest.fail "expected my secret"
  | Some t ->
    let s_hs_traffic = Handshake_crypto13.derive_secret t "s hs traffic" log in
    Alcotest.check cs __LOC__ s_hs_traffic_secret s_hs_traffic ;
    let key, iv = Handshake_crypto13.traffic_key cipher s_hs_traffic in
...
let derive_traffic_keys () =
  ...
  | None -> Alcotest.fail "expected some secret"
  | Some t ->
    let c_ap_traffic' = Handshake_crypto13.derive_secret t "c ap traffic" log in
    Alcotest.check cs __LOC__ c_ap_traffic c_ap_traffic' ;
    let key, iv = Handshake_crypto13.traffic_key cipher c_ap_traffic' in
    Alcotest.check cs __LOC__ app_read_key key ;
    Alcotest.check cs __LOC__ app_read_iv iv ;
    let s_ap_traffic' = Handshake_crypto13.derive_secret t "s ap traffic" log in
    Alcotest.check cs __LOC__ s_ap_traffic s_ap_traffic' ;
    let key, iv = Handshake_crypto13.traffic_key cipher s_ap_traffic' in
...
let processed_payload () =
  let buf =
    ...
    Cstruct.set_uint8 t 0 (Packet.content_type_to_int Packet.HANDSHAKE) ;
...
let self_signature () =
  match
    Handshake_common.signature TLS_1_3
      ~context_string:"TLS 1.3, server CertificateVerify"
      log None [ `RSA_PSS_RSAENC_SHA256 ] private_key
  with
  | Error _ -> Alcotest.fail "expected sth"
  | Ok data ->
    match Handshake_common.verify_digitally_signed TLS_1_3
            ~context_string:"TLS 1.3, server CertificateVerify"
...
let binder () =
  ...
  | Some s ->
    ...
    Alcotest.check cs __LOC__ prk (Handshake_crypto13.derive_secret s "res binder" Cstruct.empty) ;
