...
* <2019-02-03 Sun>
- resumption KDF, works with example traces now!
- still missing: keyupdate, post-handshake auth, 0rtt
- resumption (binder key) works now with gnutls and firefox, still need to fiddle it through
* <2019-02-07 Thu>
- hook up resumption (send PreSharedKey from server, avoid to send certificate, CV, CR if resumed session)
- use correct early_secret
- TODO still unclear what to do with a TLS session that a client opened, successfully completed, and immediately closed
  - atm (since finished + alert may be in the same tcp frame) this session does not exist in tls_lwt etc.
  - gnutls-cli when started with -r behaves in this way, and it is a good idea to be able to count + cache this session
- early data, from RFC:
  - sessionticket needs to include early_data extension
  - client sends early_data extension in client_hello
  - uses first psk binder for early_data
  - if server accepts this (by sending early_data ext in server_hello), needs to derive key and switch only when receiving endofearlydata to hs_ctx
  - if server rejects, needs to trial decrypt with hs_ctx until it read finished (and throw away the pieces in between)
* <2019-02-08 Fri>
- more 0rtt design thoughts, and steps
  a) extend sessionticket with earlydata
  b) extend hello HS with end_of_early_data, also kdf / install decryptor
   -> needs a new state in handshake13_server
  c) extend engine to handle appdata earlier in case of 0rtt
  d) trial decryption for client finished when not in 0rtt, but client sent 0rtt
* <2019-02-14 Thu>
resumption: should each PSK only be used once? should a resumption HS contain a sessionticket?
what are the security-related issues with 0rtt?
- not forward secure (only using client random)
- can be replayed
  - a _client_ should not sent identifiable data (cookies!?)
  - replay attacks possible (0rtt contains "delete", attacker can replay "delete")
  --> application needs to be able to discover whether received data was 0rtt or not
  --> either via epoch? or via another mechanism
  --> esp. if 0rtt over a resumed, client authenticated session!?
- mitigation:
  - use-only-once ticket (as database key)
  - record "unique" id (don't include everything..) of client hellos (avoiding replay) -- until max_lifetime exceeded
- related: self-contained(&encrypted) session tickets vs those with database key
  - configuration option?

client side
- needs "configuration option" whether 0rtt is desired
  - this may be part of val client : Config.client -> (state * Cstruct.t)
  - as a ?0rtt:Cstruct.t
  --> information whether 0rtt was successfully sent must be preserved
   --> i.e. if server denies, the client may need to retransmit the data (in one form or another, see alpn)
* <2019-02-15 Fri>
- read (IETF tls WG) "[TLS] Server validation of a second ClientHello" thread (HRR, which exts to use (first or second CH), what to verify?)
 -> our server doesn't check CH1 and CH2 (apart from cipher ++ group (used by log))
 -> our server uses CH2 exts
 --> we _could_ put more stuff into the cookie (to preserve stateless HRR ++ checking) maybe in the future?
- X25519 hacl
* <2019-02-18 Mon>
- ticket and lifetime
  - server selects a lifetime (in seconds, uint32)
  - server selects a ticket_age_add (uint32, obfuscated)
  - client sends obfuscated_ticket_age: in ms, delta between ticket arrival and use
 -> ok, [stateless session ticket story] server can put server time into ticket (encrypted)
  -> then computes adjusted_creation_time = server_time + RTT
  -> client reports adjusted_creation_time (encrypted), and ticket_age
  -> server can check their _current_ wall clock time against ticket_age + adjusted_creation_time for checking
-> server needs time getting passed in
 -> what about different session ticket approaches? atm, it's recorded in epoch and can be retrieved
  -> alternatives include: passing in a polyvar to config (plus specify size of cache (full/CH cache))
 -> pick one, expose it properly, abstract if there's demand
* <2019-02-19 Tue>
reading IETF TLS WG threads on 0RTT, specifically:
- [TLS] [PSA] 0-RTT tolerance is mandatory
- [TLS] 0-RTT encrypted data limits
- [TLS] Closing on 0-RTT
- also RFC sec 4.2.10 and D.3

resulting in:
- if server is configured for 0rtt, and client sends 0rtt
(a) key material matches and 0rtt is accepted
(b) does not match and trial decryption is done
both only ever up to configured max_size_limit! (not the one from the session ticket)

- if server is not configured for 0rtt, client sends it ~> failure (violating 4.2.10)
* <2019-02-20 Wed>
- downgrade (tls13-downgrade)
- P256 fiat
* <2019-02-21 Thu>
- cleanups
- 0rtt bytes
* <2019-02-22 Fri>
- really limit bytes of 0rtt
- TODO when should a server be able to send application data? according to spec, it is after its first flight
 - but that means there's no EPOCH yet :/
 - and if the server demanded client authentication, that hasn't happened yet
--> maybe install epoch earlier, and wait until client's second flight if client authentication is enabled?
- general semantics of blocking etc:
  - effectful layer call handle_tls / send_application_data
  - the former changes state, the latter uses can_handle_appdata (and returns None if not)
  - effectful layer blocks in accept/connect (via drain_handshake) until handshake_in_progress == false
  - reneg as well calls drain_handshake to wait until handshake is completed
  --> a t that is not in established should never leak (it does in the reneg case, though, since it may be shared to another task)
- whenever internal state changes to Established, a new session is prepended to the state, accessible via epoch
 -> a t in `InitialEpoch never escapes!
- in 1.0/1.1/1.2 this is slightly wrong, since while a hellorequest / clienthello was sent, the handshake is not required to happen next (there may be any appdata in transit, and we may as well send some more appdata)
 -> but it's a useful semantics!
- now 1.3 walks into the door, where application data can be send/received earlier:
  - client side may use 0rtt and just send data after clienthello
  - server side may send after their finished any data
    - what about keyupdate and post-handshake auth?
      - keyupdate shouldn't affect
      - post-hs auth is initiated by the server (sending a certificateverify), support signaled by client (client hello extension)
       -> should be blocking in the server (+new epoch, drop old data to distinguish which was authenticated)
     - in theory, multiple post-hs auth may be in transit at the same time (ctx field), but where is this useful for an application programmer?
  - epochs are also more and earlier:
    - earlydata (not ephemeral)
    - hs (ephemeral)
    - appdata (ephemeral)
  -> this means that write is allowed earlier, read as well
  --> epoch needs to be installed earlier! as soon as read/write works, an epoch should be available
   --> should it be tagged with "0rtt" etc.?
- client API for connect including 0rtt which is denied
 -> first step: do we have a ticket that allows 0rtt?
 -> second step: allow write early - but what to return here?
- drop is only for "we'd like a new epoch for client auth"
- on recv side, you block on read (or not), handle the data (using epoch to inspect which if you like), and call read again (potentially in a new epoch)
- or accept / server_of_fd returns not only a t, but also a cstruct.t option -- then accept can only return when end of early data is received (but we could want to send application data earlier, directly after our finished)
-- the 1.3 model suggests we've separate can_x_application_data for send and receive
- whenever acc/conn returns:
  accept : server -> ?read_zero_rtt:bool -> fd -> (t * sockaddr * buffer option) io
              (* or via config? *) (* where would this be if zero_rtt is via config? *)
  could: accept_0rtt : server -> fd -> (t * sockaddr * buffer option)
  connect : client -> ?zero_rtt:buffer -> host * port -> t * zero_rtt_sent:bool io
    actually:
       Engine.client : Config.client -> Cstruct.t option -> (state * Cstruct.t *)
 - we can call read and write on it
 - then we can write on it, and it has an epoch
 - read doesn't read 0rtt. if you want it, you get all at once.
* <2019-02-23 Sat>
- TODO embed kex info into epoch!
- what should session.state contain?
 - why do we keep old sessions around? should it instead of a list be an option?
 - track epoch/crypto context numbers?
   - None ~> Some `Handshake ~> Some `AppX ~> Some `AppX+1
   - but that's different for the two peers, can a API client make use out of that info?
- SHA384 fixes (empty psk must be hash len, certificateverify uses the same hash)
 - TODO is the latter necessary? can PSS-SHA256 be used with AES-256-GCM-SHA384?
* <2019-02-25 Mon>
- server side things
  - preserve resumption_secret for future session tickets (computed in _finished)
    - track nonce
  - respect certificate_signature_algorithms in agreed_certs
  - post-handshake authentication
  - keyupdate
  - API: 0rtt (see above discussion)
- working on client side
  - handshake
  - hrr
- still needs:
  - send ccs for middlebox
  - client authentication
  - ee data into epoch
  - psk, 0rtt
* <2019-02-26 Tue>
- tlsfuzzer testing https://gist.github.com/hannesm/97824e6a634e5f4209b018c119ecede7
- ee alpn into epoch
- client psk:
  - make psk from session ticket available!?
  - need [(identity, obf_ts, old ts, current ts), secret] as input (maybe epoch)
    - could have multiple PSK!?
  - computes binder (using CH), and sends it out
 - missing lifetime + ticket extension (0RTT) -- now done!
* <2019-02-27 Wed>
- TODO BUG respect and record server's `Hostname (and report in epoch)
- TODO ticket_cache, cached_session, session_cache Config thingies:
  - session_cache is ID -> epoch option (for server)
  - cached_session is epoch option (for client)
  - now, tickets are a bit more intricate:
    - need timestamp and compute time deltas
    - server needs to include (lifetime, early_data_bytes ext)
    - client _when sending_ needs obfuscated_age + delta, verify lifetime
       or take timestamp * psk * lifetime as input, and compute what's needed
    - client _when receiving_ needs to take timestamp and output lifetime
    - client may send multiple tickets (when is this useful?)
    - currently a record { lookup ; ticket_granted ; lifetime ; timestamp }
      -> application passes a callback instead of requesting via epoch on a session
        -> because the new session ticket may appear at any time, not under control of client
    ~> added lifetime and early_data to psk13!
-> ts needed to preserve:
   ticket_issuance, obfuscation, lifetime
- TODO temporarily disabled padding since it's awkwardly put by Writer at the end of the buffer
  ~~> PSK needs to go last!
- TODO HRR and resumption
- client authentication (unfinished, TODO doesn't respect server's CR exts)
* <2019-02-28 Thu>
- TODO can_handle_appdata story in respect with 1.3 and 0rtt
- cleanups (remove add_to_cache/find_in_cache), don't assemble clienthello multiple times
- downgrade magic numbers cleanups
---
- client zerortt (+HRR+resume), sessionticket lifetime checks
- client send CCS (middlebox mode)
- API zerortt
- check fragment being empty
- post-HS authentication
- keyupdate
- extensions (esp cert_sig_alg, others?)
- OCSP/..
- server side ticket issuance and reusability (stateless tickets, nonce, etc.)
- general: when should the session be in the state (and epoch return sth useful)?
  - the whole peeking whether the top is a `TLS13 isn't so nice
- padding policy (or general padding API)
- looked at diff master..tls13 (minimised, found some issues)
